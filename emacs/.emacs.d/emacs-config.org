...
* Hide ModeLine
#+begin_src emacs-lisp
  (use-package hide-mode-line :ensure t :hook (vterm-mode . hide-mode-line-mode)(dashboard-mode . hide-mode-line-mode))
#+end_src
...
:ensure nil
:load-path "~/mu4e-dashboard"
:custom (mu4e-dashboard-file "~/mu4e-dashboard/side-dashboard.org"))
   #+end_src
* GPG
  #+begin_src emacs-lisp
...
* ORG Blog
  #+begin_src emacs-lisp
      (defun publish ()
        "Publish to gi-yt.github.io"
        (interactive)
        (setq file (buffer-file-name))
        (setq tags (read-string "Enter all tags seperated by comma no spaces: "))
        (shell-command-to-string (format "bash ~/.local/bin/pandoc-org-md-blog %s" file))
        (shell-command-to-string (format "alacritty -e bash ~/pages/test.sh %s" tags))
    )
  #+end_src
